...
## 2026-05-21 00:20 Asia/Shanghai - Preserve remote logs and continue maintenance docs

- User request:
  - Continue improving logs and maintenance documentation.
- Intent / context:
  - A previous documentation push accidentally copied older local docs over the remote branch and shortened `docs/OPERATION_LOG.md` / `docs/MAINTAINER_CONTEXT.md`.
  - This entry records the corrective pass: restore the remote long-form documents, keep the new handoff docs, regenerate the archive, and push a follow-up correction.
- Files changed:
  - `README.md`
  - `docs/AI_HANDOFF.md`
...
- Commands / validation:
  - `git show HEAD^:docs/OPERATION_LOG.md | wc -l`
  - `git show HEAD^:docs/MAINTAINER_CONTEXT.md | wc -l`
  - `git checkout HEAD^ -- README.md docs/MAINTAINER_CONTEXT.md docs/OPERATION_LOG.md`
  - regenerated archive from current docs
  - `git diff --stat`
- Version:
...
- Push:
  - pending at time of entry
- Result summary:
  - Restores the original long operation log and maintainer context instead of replacing them.
  - Keeps the 2026-05-20 production HAOS upgrade/repair handoff docs.
  - Regenerates the handoff archive after the restore.
- Next handoff:
...
- User request:
  - Upgrade the visible HAOS `OpenClaw HA Add-on` for the 2026-05-20 upstream OpenClaw release.
  - Make HAOS detect the update, build/push images, pull the update on HAOS, run it, clean old images/backups, fix logs, and document everything for future AI maintainers.
- Intent / context:
  - The user was looking at production add-on `sunboss/openclaw-ha-addon`, not this Rust rewrite repo.
  - HAOS originally showed `OpenClaw HA Add-on` at `2026.04.25.1`.
  - A duplicate Rust rewrite add-on had been temporarily installed and later removed to avoid confusion.
- Files changed in this Rust rewrite documentation pass:
  - `README.md`
  - `docs/AI_HANDOFF.md`
  - `docs/HAOS_MAINTENANCE_RUNBOOK.md`
  - `docs/RELEASE_2026-05-20.md`
  ...
  - `docs/archives/openclaw-haos-maintenance-docs-*.zip`
  - `docs/archives/openclaw-haos-maintenance-docs-*.tar.gz`
- Production repository changes already pushed to `sunboss/openclaw-ha-addon`:
  - `8bb2830` - upgraded to upstream OpenClaw `2026.5.18`
  - `692ab68` - fixed CI dependency install with `pnpm`
  - `5301ffc` - hardened HAOS Control UI config, version `2026.05.20.2`
- Commands / validation:
  - GitHub Actions run `26158357337` succeeded: `meta`, `linux/amd64`, `linux/arm64`, `manifest`.
  - `docker manifest inspect ghcr.io/sunboss/openclaw-ha-addon:2026.05.20.2`
  - `ha store reload`
...
- Version:
  - Production add-on: `2026.05.20.2`
  - Rust rewrite repo documentation only; no Rust rewrite release.
- Commit:
  - Production latest: `5301ffc`
  - Documentation package commit: `a553771`, followed by a corrective docs preservation commit.
- Push:
  - Production `sunboss/openclaw-ha-addon` pushed successfully.
  - Rust rewrite documentation package pushed to `sunboss/OpenClawHAOSAddon-Rust`.
- Result summary:
  - HAOS shows `version=2026.05.20.2`, `version_latest=2026.05.20.2`, `update_available=false`.
  - Running image is `ghcr.io/sunboss/openclaw-ha-addon:2026.05.20.2`.
  - Gateway returns `HTTP/2 200`.
  - Control UI insecure auth flags are false.
  - Agent thinking defaults are `medium` except `addon-developer=high`.
  - Expired OpenAI Codex OAuth profiles were backed up and removed.
  - Obsolete Hermes HAOS cron job `cc1cf5dc-f611-47a4-925e-5d6339934e9f` was disabled.
  - Disk after cleanup was about `57.4G` used and `51.9G` available.
- Next handoff:
  - If the user wants Codex/OpenAI agent turns inside OpenClaw again, they must re-authenticate in the UI because old refresh tokens were invalid.
  - If future logs show cron timeouts, check `openclaw cron list/status` first; current expected state is no enabled cron jobs.
  - Do not re-add the duplicate Rust rewrite add-on to HAOS unless explicitly asked.
