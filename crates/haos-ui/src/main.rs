...
#[derive(Clone)]
struct CachedSnapshot { snapshot: SystemSnapshot, health_ok: Option<bool> }

#[derive(Clone)]
struct AppState { cache: Arc<RwLock<Option<CachedSnapshot>>> }
...
#[derive(Clone, Debug)]
struct SystemSnapshot { openclaw_uptime: String }

impl PageConfig {
    fn from_env() -> Self {
        let config = load_runtime_config();
        let gateway_token = config.as_ref().and_then(|v| string_path(v, "gateway.auth.token")).unwrap_or_default();
        Self {
...
fn load_json_file(path: PathBuf) -> Option<serde_json::Value> {
    fs::read_to_string(path).ok().and_then(|text| serde_json::from_str::<serde_json::Value>(&text).ok())
}

fn runtime_config_path() -> PathBuf {
    env::var("OPENCLAW_CONFIG_PATH").map(PathBuf::from).unwrap_or_else(|_| PathBuf::from("/config/.openclaw/openclaw.json"))
}

fn load_runtime_config() -> Option<serde_json::Value> { load_json_file(runtime_config_path()) }

fn string_path(config: &serde_json::Value, path: &str) -> Option<String> {
    let mut current = config;
    for part in path.split('.').filter(|part| !part.is_empty()) { current = current.get(part)?; }
    current.as_str().map(|value| value.trim().to_string()).filter(|value| !value.is_empty())
}

fn env_value(key: &str, fallback: &str) -> String { env::var(key).unwrap_or_else(|_| fallback.to_string()) }

async fn fetch_openclaw_health() -> Option<bool> {
    let mut stream = timeout(Duration::from_millis(1500), TcpStream::connect("127.0.0.1:48099")).await.ok()?.ok()?;
    timeout(Duration::from_millis(1500), stream.write_all(b"GET /readyz HTTP/1.1\r\nHost: 127.0.0.1\r\nConnection: close\r\n\r\n")).await.ok()?.ok()?;
    let mut response = String::new();
    timeout(Duration::from_millis(1500), stream.read_to_string(&mut response)).await.ok()?.ok()?;
    Some(response.starts_with("HTTP/1.1 200") || response.starts_with("HTTP/1.0 200"))
}

fn format_duration(seconds: u64) -> String {
    let days = seconds / 86_400; let hours = (seconds % 86_400) / 3_600; let minutes = (seconds % 3_600) / 60;
    if days > 0 { format!("{days} 天 {hours} 小时 {minutes} 分") } else if hours > 0 { format!("{hours} 小时 {minutes} 分") } else { format!("{minutes} 分") }
}

fn process_uptime(pid: &str) -> Option<String> {
    if pid.trim().is_empty() || pid == "-" { return None; }
    let output = Command::new("ps").args(["-p", pid, "-o", "etimes="]).output().ok()?;
    if !output.status.success() { return None; }
    let seconds = String::from_utf8_lossy(&output.stdout).trim().parse::<u64>().ok()?;
    Some(format_duration(seconds))
}

async fn collect_system_snapshot() -> SystemSnapshot {
    tokio::task::spawn_blocking(|| {
        let openclaw_uptime = process_uptime(&pid_value("openclaw-gateway")).or_else(|| process_uptime(&pid_value("openclaw-node"))).unwrap_or_else(|| "不可用".to_string());
        SystemSnapshot { openclaw_uptime }
    }).await.unwrap_or_else(|_| SystemSnapshot { openclaw_uptime: "不可用".to_string() })
}

fn pid_value(name: &str) -> String {
    fs::read_to_string(format!("/run/openclaw-rs/{name}.pid")).ok().map(|v| v.trim().to_string()).filter(|v| !v.is_empty()).unwrap_or_else(|| "-".to_string())
}

fn access_mode_label(mode: &str) -> &'static str {
    match mode { "local_only" => "仅本机访问", "lan_https" => "局域网 HTTPS", "lan_reverse_proxy" => "局域网反向代理", "tailnet_https" => "Tailnet HTTPS", "custom" => "自定义", _ => "当前模式" }
}

fn js_string(value: &str) -> String { serde_json::to_string(value).unwrap_or_else(|_| "\"\"".to_string()) }
...
async fn run_openclaw_command(args: Vec<&'static str>) -> Result<OpenClawCommandResult, String> {
    tokio::task::spawn_blocking(move || {
        let output = Command::new("openclaw").args(&args).output().map_err(|err| format!("无法执行 openclaw：{err}"))?;
        Ok(OpenClawCommandResult { ok: output.status.success(), stdout: String::from_utf8_lossy(&output.stdout).trim().to_string(), stderr: String::from_utf8_lossy(&output.stderr).trim().to_string() })
    }).await.map_err(|err| format!("后台任务失败：{err}"))?
}

async fn approve_latest_device() -> impl IntoResponse {
    match run_openclaw_command(vec!["devices", "approve", "--latest"]).await {
        Ok(result) if result.ok => Json(serde_json::json!({"ok": true, "message": if !result.stdout.is_empty() { result.stdout } else if !result.stderr.is_empty() { result.stderr } else { "已执行 openclaw devices approve --latest".to_string() }})),
        Ok(result) => Json(serde_json::json!({"ok": false, "message": if !result.stderr.is_empty() { result.stderr } else if !result.stdout.is_empty() { result.stdout } else { "授权失败，请先检查待批准设备列表".to_string() }})),
...
async fn list_devices() -> impl IntoResponse {
    match run_openclaw_command(vec!["devices", "list", "--json"]).await {
        Ok(result) if result.ok => {
            let output = if result.stdout.is_empty() { "没有返回设备数据".to_string() } else { match serde_json::from_str::<serde_json::Value>(&result.stdout) { Ok(json) => serde_json::to_string_pretty(&json).unwrap_or_else(|_| result.stdout.clone()), Err(_) => result.stdout.clone() } };
            let pending = serde_json::from_str::<serde_json::Value>(&result.stdout).ok().and_then(|value| value.get("pending")?.as_array().map(|items| items.len())).unwrap_or(0);
            let paired = serde_json::from_str::<serde_json::Value>(&result.stdout).ok().and_then(|value| value.get("paired")?.as_array().map(|items| items.len())).unwrap_or(0);
            Json(serde_json::json!({"ok": true, "message": format!("已读取设备列表：pending {}，paired {}", pending, paired), "output": output }))
        }
        Ok(result) => Json(serde_json::json!({"ok": false, "message": if !result.stderr.is_empty() { result.stderr.clone() } else { "读取设备列表失败".to_string() }, "output": if result.stdout.is_empty() { result.stderr } else { result.stdout }})),
...
mod tests {
    ...
    fn sample_page_config() -> PageConfig {
        PageConfig { addon_version: "2026.04.15.1".to_string(), access_mode: "lan_https".to_string(), gateway_url: String::new(), openclaw_version: "2026.4.14".to_string(), https_port: "18789".to_string(), gateway_token: "tok_test_12345678".to_string() }
    }
    ...
    fn home_page_keeps_only_single_page_controls() {
        let config = sample_page_config();
        let snapshot = SystemSnapshot { openclaw_uptime: "35 分".to_string() };
...
fn primary_link_button(label: &str, id: &str, onclick: &str) -> String { format!(r##"<a class="btn primary" id="{id}" href="#" target="_blank" rel="noopener noreferrer" onclick="{onclick}">{label}</a>"##) }

fn shell_window_button(label: &str, command: &str) -> String {
    let command = html_attr_escape(&js_string(command));
    format!(r#"<button class="btn secondary" type="button" onclick="ocOpenShellWindow({command})">{label}</button>"#)
}

fn kv_row(label: &str, value: &str) -> String { format!(r#"<div class="kv-row"><span class="kv-label">{label}</span><span class="kv-value">{value}</span></div>"#) }

fn summary_strip(title: &str, value: &str, sub: &str, tone: &str) -> String { format!(r#"<article class="summary-strip-card {tone}"><div class="summary-strip-title">{title}</div><div class="summary-strip-value">{value}</div><div class="summary-strip-sub">{sub}</div></article>"#) }

fn service_badge(label: &str, pid: &str) -> String {
    let (state_class, state_text, pid_text) = if pid != "-" { ("is-online", "在线", format!("PID {pid}")) } else { ("is-offline", "待启动", "未检测到 PID".to_string()) };
    format!(r#"<article class="service-badge {state_class}"><div class="service-badge-top"><span class="service-name"><span class="svc-dot"></span>{label}</span><span class="service-state">{state_text}</span></div><div class="service-meta">{pid_text}</div></article>"#)
}
...
fn gateway_token_section(token: &str) -> String {
    ...
    format!(r#"<section class="card"><div class="card-head compact"><div><div class="eyebrow">访问令牌</div><h2>显示 Gateway Token</h2></div></div><div class="note-box"><strong>只在需要时显示</strong><p>这个令牌用于直接进入原生 OpenClaw Gateway。平时建议保持隐藏，只在调试和手动连接时查看。</p></div><div class="token-section"><div class="token-header"><span>Gateway Token</span><span>请勿分享给不受信任设备</span></div><div class="token-row"><code class="token-val" id="ocTokenVal">{masked}</code><button class="btn" id="ocTokenToggleBtn" type="button" onclick="ocToggleToken()">显示</button><button class="btn" type="button" onclick="ocCopyToken(this)">复制</button></div></div><script>(function(){{var t="{tok_escaped}";window.ocToggleToken=function(){{var v=document.getElementById("ocTokenVal"),b=document.getElementById("ocTokenToggleBtn");if(!v||!b)return;if(b.dataset.vis==="1"){{v.textContent="••••••••"+t.slice(-8);b.textContent="显示";b.dataset.vis="";}}else{{v.textContent=t;b.textContent="隐藏";b.dataset.vis="1";}}}};window.ocCopyToken=function(btn){{var orig=btn.textContent;function done(){{btn.textContent="已复制 ✓";setTimeout(function(){{btn.textContent=orig;}},1500);}}function fb(){{try{{var ta=document.createElement("textarea");ta.value=t;ta.style.cssText="position:fixed;opacity:0;top:0;left:0;width:1px;height:1px";document.body.appendChild(ta);ta.focus();ta.select();var ok=document.execCommand("copy");document.body.removeChild(ta);if(ok){{done();}}else{{alert("Token: "+t);}}}}catch(e){{alert("Token: "+t);}}}}if(navigator.clipboard){{navigator.clipboard.writeText(t).then(done,fb);}}else{{fb();}}}};}})()</script></section>"#, masked = masked, tok_escaped = tok_escaped)
}
...
fn render_shell(config: &PageConfig, title: &str, subtitle: &str, content: &str) -> Html<String> {
    ...
    let https_port = js_string(&config.https_port);
    Html(format!(
        r##"<!doctype html>
<html lang="zh-CN">
<head>
