...
fn build_ingress_router(state: AppState) -> Router {
    Router::new()
        ...
        .route("/health", get(proxy_readyz))
        .route("/healthz", get(proxy_healthz))
        .route("/readyz", get(proxy_readyz))
...
async fn proxy_healthz(State(state): State<AppState>) -> impl IntoResponse {
    proxy_gateway_probe(&state, "/healthz").await
}

async fn proxy_readyz(State(state): State<AppState>) -> impl IntoResponse {
    proxy_gateway_probe(&state, "/readyz").await
}
...
async fn token_file() -> impl IntoResponse {
    let public_share_dir =
        env::var("PUBLIC_SHARE_DIR").unwrap_or_else(|_| "/run/openclaw-rs/public".to_string());
    let path = format!("{public_share_dir}/gateway.token");
...
async fn cert_file() -> impl IntoResponse {
    let public_share_dir =
        env::var("PUBLIC_SHARE_DIR").unwrap_or_else(|_| "/run/openclaw-rs/public".to_string());
    let path = format!("{public_share_dir}/openclaw-ca.crt");
...
async fn file_response(path: String, content_type: &str) -> impl IntoResponse {
    match fs::read(&path) {
...
async fn proxy_gateway_probe(state: &AppState, path: &str) -> Response<Body> {
    let target = format!("{}{}", state.gateway_http_base, path);
